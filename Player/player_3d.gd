...
# Sprites
@onready var animated_sprite_izra: AnimatedSprite3D = $AnimatedSpriteIzra
@onready var animated_sprite_ishtu: AnimatedSprite3D = $AnimatedSpriteIshtu
@onready var animated_sprite_bob: AnimatedSprite3D = $AnimatedSpriteBob
@onready var animated_sprite_samurai: AnimatedSprite3D = $AnimatedSpriteSamurai
@onready var animated_sprite_smash_knight: AnimatedSprite3D = $AnimatedSpriteSmashKnight
@onready var damage_animation: AnimatedSprite3D = $DamageAnimation

# Health self-heal (wolverine) timer
...
var hp = 0
var time = 0
var active_sprite: AnimatedSprite3D = null
var flip_anim = false # flag to flip / mirror the animation 
const SPEED = 5.0
...
## Fireball
# -----------------------------------------------------------
var fireball = preload("res://Player/Attack/fireball_3d.tscn")
# Attack Nodes
...
# Last known direction of player
var last_direction = Vector3.UP
...
func get_random_target() -> Vector3:
	...
	if enemy_close.size() > 0:
		# Return a random enemy's global position
		return enemy_close.pick_random().global_transform.origin
	else:
		# Use the last known direction when there are no enemies and the player is not moving
		if last_direction.x < 0:  # facing left
			return global_transform.origin + Vector3(-10, 0, 0)
		elif last_direction.x > 0:  # facing right
			return global_transform.origin + Vector3(10, 0, 0)
		elif last_direction.y < 0:  # facing up (in 2D, adjust to vertical in 3D)
			return global_transform.origin + Vector3(0, 0, -10)
		elif last_direction.y > 0:  # facing down
			return global_transform.origin + Vector3(0, 0, 10)
		else:
			# Default to shooting in a specified direction (upwards in 3D space)
			return global_transform.origin + Vector3(0, 1, 0)  # Adjust this if you want a different default direction
...
func _on_ice_spear_attack_timeout() -> void:
	if icespear_ammo > 0:
...
func _on_suriken_timer_timeout() -> void:
	suriken_ammo += suriken_baseammo + additional_attacks
	suriken_attack_timer.start()


func _on_suriken_attack_timer_timeout() -> void:
	if suriken_ammo > 0:
...
func _on_hurt_box_3d_hurt(damage: int, angle: Vector3, knockback_amount: float) -> void:
